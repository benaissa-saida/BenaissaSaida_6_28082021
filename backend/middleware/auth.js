...
module.exports = (req, res, next) => {
    try{
        const token = req.headers.authorization.split(' ')[1]; // on récupére les différentes données après le bearer 
        const decodedToken = jwt.verify(token, 'RANDOM_TOKEN_SECRET'); // On utilise la fonc verify pour décoder le token si pas valide => erreur
